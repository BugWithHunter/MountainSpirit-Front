<template>
    <div class="report-state" v-if="report">
      <h3>회원 신고 상세 정보</h3>
      <p><strong>신고사유:</strong> {{ report.reason }}</p>
      <p><strong>회원 이름:</strong> {{ report.memName }}</p>
      <p><strong>크루 ID:</strong> {{ report.crewId ?? '-' }}</p>
      <p><strong>등산 횟수:</strong> {{ report.climbCnt }}</p>
      <p><strong>점수:</strong> {{ report.score }}</p>
      <p><strong>신고당한 횟수:</strong> {{ report.reportCount }}</p>
  ...
  const fetchReport = async (id) => {
    try {
      ...
      console.error('회원 신고 상세 불러오기 실패:', err);
    }
  };
